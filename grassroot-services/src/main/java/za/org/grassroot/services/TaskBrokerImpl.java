...
public class TaskBrokerImpl implements TaskBroker {
	...
	public List<TaskDTO> searchForTasks(String userUid, String searchTerm) {
		...
		User user = userRepository.findOneByUid(userUid);
		List<Event> events = searchEventsByTextTerm(user, searchTerm);
		Set<TaskDTO> taskDTOs = resolveEventTaskDtos(events, user, null);

		List<LogBook> toDos = searchLogBooksByTextTerm(user, searchTerm);
    ...
    private List<LogBook> searchLogBooksByTextTerm(User user, String searchTerm) {
        FullTextEntityManager fullTextEntityManager = Search.getFullTextEntityManager(entityManager);

        // create the query using Hibernate Search query DSL
        QueryBuilder queryBuilder = fullTextEntityManager.getSearchFactory().buildQueryBuilder()
                .forEntity(LogBook.class)
                .get();

        // a very basic query by keywords
        Query query = queryBuilder
                .keyword()
                .onField(LogBook.LUCENE_FIELD_MESSAGE)
                .matching(searchTerm)
                .createQuery();

        // wrap Lucene query in an Hibernate Query object
        FullTextQuery jpaQuery = fullTextEntityManager.createFullTextQuery(query, LogBook.class);
        jpaQuery.enableFullTextFilter("idsFilter").setParameter("ids", Sets.newHashSet(1L));

        // execute search and return results (sorted by relevance as default)
        return jpaQuery.getResultList();
    }

    private List<Event> searchEventsByTextTerm(User user, String searchTerm) {
